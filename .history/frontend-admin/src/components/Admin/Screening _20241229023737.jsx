  ...
  const fetchShowtimesForCinema = async (cinemaId, cinemaRooms) => {
    try {
      ...
      const filteredShowtimesForSelectedDate = filteredShowtimes.filter((showtime) => {
        console.log("Showtime days: ", showtime.days);  // Log các giá trị trong days
        return showtime.days.some((day) => {
          const showtimeDay = moment(day).format("YYYY-MM-DD");
          console.log("Comparing with selected date: ", showtimeDay, selectedDate); // Log so sánh ngày
          return showtimeDay === selectedDate;
        });
      });
