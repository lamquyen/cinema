    ...
    if (selectedCinema && selectedRoomId) {
      axios
        ...
        .then((response) => {
          ...
          // Kiểm tra kiểu dữ liệu và định dạng của showtime.date và selectedDate
          console.log("Showtimes days:", showtimes);
          console.log("Selected Date:", selectedDate);
  
          // Lọc các showtimes theo ngày đã chọn và phòng đã chọn
          const selectedDateRoomShowtimes = showtimes.filter((showtime) => {
            // Đảm bảo showtime.date và selectedDate đều là đối tượng moment
            const showtimeDate = moment(showtime.date);
            const selectedMomentDate = moment(selectedDate);
            
            console.log("Comparing dates:", showtimeDate.format(), selectedMomentDate.format());
  
            // So sánh các ngày
            return showtimeDate.isSame(selectedMomentDate, "day");
          });
