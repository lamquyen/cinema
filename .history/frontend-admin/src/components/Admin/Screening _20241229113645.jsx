    ...
    if (selectedRoom?.id === room.id) {
      ...
      if (selectedCinema) {
        ...
        fetchShowtimesForCinema(cinemaId, [room.id]).then((showtimes) => {
          ...
          // Lưu suất chiếu đã lọc vào state roomFilteredShowtimes
          setRoomFilteredShowtimes((prev) => ({
            ...prev,
            [room.id]: filteredShowtimesForRoom,
          }));
        });
      }
    }
...
>
  ...
  <p>
    Số suất chiếu:{" "}
    {roomFilteredShowtimes[room.id]?.length > 0
      ? roomFilteredShowtimes[room.id].length
      : 0}
    /4
  </p>
</div>
