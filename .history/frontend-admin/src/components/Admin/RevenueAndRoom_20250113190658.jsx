...
function RevenueAndRoom() {
  ...
  const calculateTotalRevenue = (data) => {
    return data.reduce(
      (total, item) => {
        ...
        const seat = Array.isArray(item.seat) ? item.seat : []; // Kiểm tra seat
        const foodNames = item.foodNames || []; // Kiểm tra foodNames
  
        console.log("Seats:", seat); // Kiểm tra seat
        ...
        total.totalSeatPrice += item.totalSeatPrice || 0;
        total.totalFoodPrice += item.totalFoodPrice || 0;
        ...
        console.log("Total after iteration:", total); // Kiểm tra tổng
  ...
  }, [bookingData]); // Đảm bảo rằng bookingData đã được cập nhật trước khi tính toán lại tổng doanh thu
