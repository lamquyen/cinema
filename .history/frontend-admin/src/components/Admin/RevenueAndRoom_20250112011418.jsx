...
function RevenueAndRoom() {
    ...
    if (selectedCinema) {
      axios
        ...
        .then((response) => {
          ...
          // Lọc các showtimes theo ngày đã chọn
          const selectedDateShowtimes = showtimes.filter((showtime) =>
            moment(showtime.date).isSame(moment(selectedDate), "day")
          );
  
          setShowtimesData(selectedDateShowtimes);
        })
        .catch((error) => {
  ...
  const selectedDateShowtimes = selectedRoomShowtimes.filter(
    (showtime) => moment(showtime.date).isSame(moment(selectedDate), 'day')
  );
