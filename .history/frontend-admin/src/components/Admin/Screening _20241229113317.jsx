...
function Screening() {
  ...
  const handleSelectRoom = (room) => {
    if (selectedRoom?.id === room.id) {
      // Nếu phòng được bấm đã được chọn, hủy chọn (tắt showtimes)
      setSelectedRoom(null);
      setFilteredShowtimes([]);
    } else {
      // Nếu chọn phòng mới, hiển thị showtimes
      ...
      if (selectedCinema) {
        ...
        fetchShowtimesForCinema(cinemaId, [room.id]).then((showtimes) => {
          ...
          // Đảm bảo luôn có 4 box-showtime
          const paddedShowtimes = Array.from({ length: 4 }).map((_, index) => {
            return filteredShowtimesForSelectedDate[index] || null;
          });

          // Cập nhật state filteredShowtimes với dữ liệu đã xử lý
          setFilteredShowtimes(paddedShowtimes);

          console.log("Selected Date: ", selectedDate);
          console.log("Filtered Showtimes: ", paddedShowtimes);
        });
      }
    }
  };
                      ...
                      <div className="showtimes">
                        {filteredShowtimes.map((showtime, index) => {
                          if (showtime) {
                            ...
                            // Nếu không có suất chiếu hoặc rỗng
                            return (
                              <div key={index} className="box-showtime">
                                <>
                                  <h5>Rạp trống</h5>
