...
function RevenueAndRoom() {
  ...
  {selectedDateShowtimes.length > 0 ? (
    selectedDateShowtimes.map((showtime) => {
      if (!pendingShowtimeIds.includes(showtime._id)) {
        setPendingShowtimeIds((prev) => [...prev, showtime._id]);
      }

      // Lọc dữ liệu booking liên quan đến showtime
      const bookingsForShowtime = showtimesData.filter(
        (booking) => booking.showtime === showtime._id
      );

      // Tính toán tổng số vé bán, sản phẩm bán kèm và doanh thu
      const totalTickets = bookingsForShowtime.reduce(
        (acc, booking) => acc + booking.seat.length,
        0
      );
      const totalFoodItems = bookingsForShowtime.reduce(
        (acc, booking) => acc + booking.foodNames.length,
        0
      );
      const totalRevenue =
        bookingsForShowtime.reduce(
          (acc, booking) => acc + booking.totalSeatPrice + booking.totalFoodPrice,
          0
        ) || 0;
