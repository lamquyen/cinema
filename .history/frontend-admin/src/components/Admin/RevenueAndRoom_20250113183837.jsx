...
// Component chính
const YourComponent = ({ showtimesData, bookingData }) => {
  const [totalRevenue, setTotalRevenue] = useState({
    totalSeatPrice: 0,
    totalFoodPrice: 0,
    totalSeat: 0,
  ...
  // Hàm xử lý khi bấm vào tính toán doanh thu
  const handleCalculateRevenue = () => {
    const bookingsForShowtimes = showtimesData.map((showtime) => {
      return bookingData.filter((booking) => booking.showtimeId === showtime._id);
    });

    const revenue = showtimesData.map((showtime, index) => {
      const bookingsForShowtime = bookingsForShowtimes[index];
      return calculateTotalRevenue(bookingsForShowtime);
    });

    const totalCalculatedRevenue = revenue.reduce(
      (total, current) => {
        total.totalSeatPrice += current.totalSeatPrice;
        total.totalFoodPrice += current.totalFoodPrice;
        total.totalSeat += current.totalSeat;
        total.totalSeatN += current.totalSeatN;
        total.totalSeatV += current.totalSeatV;
        total.totalSeatC += current.totalSeatC;
        total.totalCombo += current.totalCombo;
        total.totalCorn += current.totalCorn;
        total.totalPepsi += current.totalPepsi;
        return total;
      },
      {
        totalSeatPrice: 0,
    ...
    setTotalRevenue(totalCalculatedRevenue); // Cập nhật state
  };
                    ...
                    <tbody>
                      {showtimesData.length > 0 ? (
                        showtimesData.map((showtime, index) => {
                          ...
                          const bookingsForShowtime = bookingData.filter(
                            (booking) => booking.showtimeId === showtime._id
                          );
                      

                          // Tính doanh thu cho showtime này
                          const revenueForShowtime =
                            calculateTotalRevenue(bookingsForShowtime);
                          ...
                          return (
                            <tr key={showtime._id}>
                              ...
                              <td>{revenueForShowtime.totalCombo}</td>{" "}
                              {/* Doanh thu combo cho showtime này */}
                              <td>
                                {revenueForShowtime.totalSeatPrice +
                                  revenueForShowtime.totalFoodPrice}{" "}
                                đ
                              </td>{" "}
