...
function Screening() {
  ...
  const [isChangeFormVisible, setIsChangeFormVisible] = useState(false); // State mới
  const [selectedShowtime, setSelectedShowtime] = useState(null); // Showtime được chọn
  ...
  const handleAddShowtimeClick = (cinemaName, roomId, cinemaId, timeSlots) => {
    ...
    // Truyền timeSlots vào ShowtimeForm khi mở form thêm lịch chiếu
    setFormTimeSlots(timeSlots); // Đảm bảo bạn tạo state cho timeSlots nếu cần
  };
    ...
    <div className="container">
      ...
      {isFormVisible && (
        <ShowtimeForm
          ...
          cinemaId={cinemaId}
          timeSlots={formTimeSlots} // Truyền timeSlots tương ứng vào ShowtimeForm
        />
      )}
    ...
    {showtime ? (
      <>
        ...
        <p style={{ fontSize: "13px" }}>
          Tên phim: <span style={{ fontSize: "13px" }}>{showtime.movie.title}</span>
        </p>
        <p style={{ fontSize: "13px" }}>
          Suất Chiếu: <span style={{ fontSize: "13px" }}>{showtime.times}</span>
        </p>
        <a
          href="#"
          className="btn-room"
        ...
        <a
          ...
          onClick={(e) => {
            ...
            handleAddShowtimeClick(
              ...
              selectedRoom?.id,
              theater.id,
              timeSlots // Truyền timeSlots vào hàm handleAddShowtimeClick
            );
          }}
        >
          Thêm lịch chiếu
