...
function Screening() {
  ...
  const handleSelectRoom = (room) => {
    if (selectedRoom?.id === room.id) {
      setSelectedRoom(null); // Bỏ chọn nếu click lại vào cùng room
      setRoomFilteredShowtimes((prev) => {
        const newRoomShowtimes = { ...prev };
        delete newRoomShowtimes[room.id]; // Xóa showtimes của phòng đã chọn
  ...
  const handleAddShowtimeClick = (cinemaId, roomId) => {
    setSelectedCinema(cinemaData.find((cinema) => cinema.id === cinemaId)); // Lưu rạp đang chọn
    setSelectedRoom(theater.rooms.find((room) => room.id === roomId)); // Lưu phòng đang chọn
  ...
  return (
    <div className="container">
      <SideBar />
      {isFormVisible && selectedRoom && (
  <ShowtimeForm
    onClose={() => setIsFormVisible(false)}
    cinemaId={selectedRoom.cinemaId} // Truyền cinemaId
    roomId={selectedRoom.id} // Truyền roomId
  />
)}
