    ...
    if (selectedCinema) {
      ...
      fetchShowtimesForCinema(cinemaId, [room.id]).then((showtimes) => {
        ...
        const filteredShowtimesForSelectedDate = showtimes.filter((showtime) => {
          const showtimeDate = moment(showtime.date).format("YYYY-MM-DD"); // Đảm bảo showtime.date là một chuỗi hợp lệ
          console.log("Showtime Date: ", showtimeDate); // Log ra để kiểm tra
