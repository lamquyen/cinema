...
function Screening() {
  ...
  const fetchShowtimesForCinema = async (cinemaId, cinemaRooms) => {
    try {
      ...
      // Log all showtimes để kiểm tra cấu trúc dữ liệu
      ...
      const filteredShowtimesForSelectedDate = filteredShowtimes.filter((showtime) => {
        // Kiểm tra ngày (ngày chỉ bao gồm phần YYYY-MM-DD)
        const showtimeDate = moment(showtime.date).format("YYYY-MM-DD");
        console.log("Showtime Date: ", showtimeDate); // Log giá trị ngày của từng showtime
        return showtimeDate === selectedDate; // So sánh với selectedDate
      });
  
      // Kiểm tra lại filteredShowtimesForSelectedDate
      console.log("Filtered Showtimes for selected date: ", filteredShowtimesForSelectedDate);
  ...
  const handleSelectRoom = (room) => {
    ...
    if (selectedCinema) {
      ...
      fetchShowtimesForCinema(cinemaId, [room.id]).then((showtimes) => {
        ...
        const filteredShowtimesForSelectedDate = filteredShowtimes.filter((showtime) =>
          showtime.days.some((day) => moment(day).format("YYYY-MM-DD") === selectedDate)
        );
