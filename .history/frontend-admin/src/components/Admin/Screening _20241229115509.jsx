...
function Screening() {
  ...
  const handleSelectRoom = (room) => {
    if (selectedRoom?.id === room.id) {
      ...
      setRoomFilteredShowtimes((prevState) => {
        const newState = { ...prevState };
        delete newState[room.id];  // Xóa showtimes của phòng khi bỏ chọn
        return newState;
      });
      return;
    }
    ...
    if (selectedCinema) {
      ...
      fetchShowtimesForCinema(cinemaId, [room.id]).then((showtimes) => {
        const filteredShowtimesForSelectedDate = showtimes.filter((showtime) =>
          showtime.days.some((day) => moment(day).isSame(selectedDate, "day"))
        );
  
        setRoomFilteredShowtimes((prevState) => ({
          ...prevState,
          [room.id]: filteredShowtimesForSelectedDate,  // Lưu showtimes của phòng
        }));
      });
    }
  };
  ...
  {/* Hiển thị suất chiếu */}
  {selectedRoom && roomFilteredShowtimes[selectedRoom.id]?.length > 0 && (
    <div className="showtimes">
      {[...Array(4)].map((_, index) => {
        const showtime = roomFilteredShowtimes[selectedRoom.id]?.[index]; // Dùng selectedRoom.id để lấy showtimes cho phòng đã chọn
        return (
