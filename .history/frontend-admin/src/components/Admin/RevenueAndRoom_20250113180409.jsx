  ...
  const calculateTotalRevenue = (data) => {
    return data.reduce(
      (total, item) => {
        ...
        const seat = Array.isArray(item.seat) ? item.seat.length : (typeof item.seat === 'number' ? item.seat : 0); // Sửa chỗ này
        ...
        total.totalCombo += combo; // Số combo
        total.totalCorn += corn; // Số corn
        total.totalPepsi += pepsi; // Số pepsi
