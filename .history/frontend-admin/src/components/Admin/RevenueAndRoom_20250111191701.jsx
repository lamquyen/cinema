...
function RevenueAndRoom() {
  ...
  useEffect(() => {
    if (pendingShowtimeIds.length > 0) {
      const idsToFetch = [...pendingShowtimeIds]; // Copy danh sách hiện tại
      setPendingShowtimeIds([]); // Xóa danh sách chờ sau khi xử lý
  
      idsToFetch.forEach((showtimeId) => {
        axios
          .get(`http://localhost:5000/api/booking/showtime/${showtimeId}`)
          .then((response) => {
            console.log(`Booking data for showtime ${showtimeId}:`, response.data);
            setBookingData((prev) => ({
              ...prev,
              [showtimeId]: response.data,
            }));
          })
          .catch((error) => {
            console.error(`Error fetching booking data for showtime ${showtimeId}:`, error);
          });
      });
    }
  }, [pendingShowtimeIds]);
              ...
              <div className="revenue">
                {selectedRoomId ? (
                  <table className="table">
                    ...
                    <tbody>
                      {selectedRoomShowtimes.length > 0 ? (
                        selectedRoomShowtimes.map((showtime) => {
                          if (!pendingShowtimeIds.includes(showtime._id)) {
                            setPendingShowtimeIds((prev) => [
                              ...prev,
                              showtime._id,
                            ]);
                          }
