...
export const getShowtimeByCinemaLocation = async (req, res) => {
  const { cinemaId } = req.params;
  const { location } = req.query; // Vị trí bên trong cinema

  try {
    if (!location) {
      return res.status(400).json({ message: 'Vui lòng cung cấp thông tin vị trí (location).' });
    }

    // Tìm các suất chiếu theo cinemaId và vị trí (location)
    const showtimes = await Showtime.find({
      'cinema._id': cinemaId, // Kiểm tra ID cinema
      'cinema.location': location, // Kiểm tra location trong cinema
    })
      .populate('movie', 'title') // Lấy thông tin phim
      .populate('cinema', 'cinemaName location') // Lấy thông tin rạp
    ...
    if (!showtimes || showtimes.length === 0) {
      return res
        .status(404)
        .json({ message: `Không tìm thấy suất chiếu tại vị trí "${location}" trong cinema.` });
    }
