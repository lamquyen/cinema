...
export const getShowtimeByTimes = async (req, res) => {
  try {
    const { times } = req.query;

    // Kiểm tra nếu tham số 'times' không được cung cấp
    if (!times) {
      return res.status(400).json({ message: 'Cần cung cấp tham số "times".' });
    }

    // Kiểm tra nếu 'times' là chuỗi hợp lệ
    if (typeof times !== 'string') {
      return res.status(400).json({ message: 'Tham số "times" phải là chuỗi.' });
    }

    console.log("Times received: ", times); // In ra để kiểm tra giá trị

    // Truy vấn model Showtime để tìm các bản ghi có trường 'times' tương ứng
    // Lưu ý rằng đây là truy vấn tìm kiếm trong trường `times`, không phải `_id`
    const showtimes = await Showtime.find({ times: times });

    if (showtimes.length === 0) {
      return res.status(404).json({ message: 'Không tìm thấy suất chiếu với thời gian này.' });
    }

    // Trả về kết quả showtimes tìm được
