...
export const getByShowtimeId = async (req,res) =>{
  try {
    ...
    // Tìm các booking với showtimeId
    const bookings = await BookingModel.find({ showtimeId }).select(
      "seat.typeSeat foodNames.name foodNames.quantity totalSeatPrice totalFoodPrice"
    );
