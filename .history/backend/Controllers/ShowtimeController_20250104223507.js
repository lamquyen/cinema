...
export const updateShowtimeByRoomId = async (req, res) => {
  const { roomId } = req.params;
  const { movie, times } = req.body;
  ...
  try {
    // Tìm Showtime theo roomId và chỉ cập nhật các trường movie và times
    const updatedShowtime = await Showtime.findOneAndUpdate(
      { room: roomId }, // tìm theo roomId
      { movie, times }, // các trường cần cập nhật
      { new: true } // trả về document đã được cập nhật
    );

    if (!updatedShowtime) {
      return res.status(404).json({ message: 'Không tìm thấy showtime cho phòng này' });
    }
