...
export const updateShowtimeById = async (req, res) => {
  const { showtimeId } = req.params; // Lấy ID của Showtime từ params
  const { movie, times } = req.body; // Lấy dữ liệu cần cập nhật từ body

  if (!movie || !times) {
    return res.status(400).json({ message: 'Cần có movie và times để cập nhật' });
  }

  try {
    // Tìm Showtime theo ID và cập nhật movie, times
    const updatedShowtime = await Showtime.findByIdAndUpdate(
      showtimeId, // tìm theo ID
      { movie, times }, // các trường cần cập nhật
      { new: true } // trả về tài liệu đã được cập nhật
    );

    if (!updatedShowtime) {
      return res.status(404).json({ message: 'Không tìm thấy Showtime với ID này' });
    }
