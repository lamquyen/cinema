...
const QuickBooking = () => {
  ...
  const fetchShowtimesByMovieId = async (movieId) => {
    try {
      ...
      // Lấy thông tin phim từ API movies (hoặc bạn đã có trong state)
      const movieResponse = await axios.get(
        `http://localhost:5000/api/movies/released`
      );
      setShowingMovies(movieResponse.data); // Lưu danh sách phim vào state
    } catch (error) {
      ...
      setShowingMovies([]); // Đặt lại danh sách phim
    }
  };
  ...
  const handleDateChange = (date) => {
    ...
    const filteredTimes = showingMovies
      ...
      .filter((showtime) =>
        showtime.days && showtime.days.some((day) => new Date(day).toLocaleDateString("vi-VN") === date) // Kiểm tra ngày có trong showtimes
      )
      .map((showtime) => showtime.times); // Lấy tất cả times của showtime
