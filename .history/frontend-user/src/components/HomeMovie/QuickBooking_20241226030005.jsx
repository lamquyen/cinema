...
const QuickBooking = () => {
  ...
  const handleDateChange = (date) => {
    ...
    // Kiểm tra xem ngày đã chọn có tồn tại trong availableTimes không
    const filteredTimes = availableTimes[date] || [];
  
    console.log("Filtered Times for", date, ":", filteredTimes); // Kiểm tra các suất chiếu cho ngày đã chọn
  
    // Cập nhật lại availableTimes cho ngày đã chọn
    setAvailableTimes({ [date]: filteredTimes });
  
    // Nếu có suất chiếu, lưu id của suất chiếu đầu tiên
    if (filteredTimes.length > 0) {
      setShowtimeId(filteredTimes[0]._id); // Lưu showtimeId của suất chiếu đầu tiên
    }
  };
