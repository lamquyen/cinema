...
const QuickBooking = () => {
  ...
  const handleDateChange = (date) => {
    ...
    // Kiểm tra các showtimes cho ngày đã chọn
    const filteredTimes = filteredShowtimes.filter(
      (showtime) => new Date(showtime.days[0]).toLocaleDateString("vi-VN") === date
    );

    // Nếu có showtimes, lưu id của showtime vào state
    if (filteredTimes.length > 0) {
      // Lưu id của suất chiếu đầu tiên (hoặc tùy theo logic của bạn)
      setShowtimeId(filteredTimes[0]._id); // Lưu vào state 'showtimeId'
    }
