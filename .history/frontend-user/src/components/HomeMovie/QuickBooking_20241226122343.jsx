...
const QuickBooking = () => {
  ...
  const isBookingValid = selectedMovie && selectedCinema && selectedDate && selectedTime && showtimeId;
  ...
  <select
    value={selectedTime}
    onChange={(e) =>
      handleTimeChange(e.target.value, e.target.options[e.target.selectedIndex].getAttribute("data-id"))
    }
  ...
  >
    ...
    {/* Lọc và hiển thị các suất chiếu phù hợp */}
    {availableTimes[selectedDate]
      ?.filter(
        (showtime) => showtime.cinemaName === selectedCinema // Chỉ suất chiếu thuộc rạp đã chọn
      )
      .map((showtime, index) => (
        <option key={index} value={showtime.time} data-id={showtime.showtimeId}>
          {showtime.time}
        </option>
      ))}
  </select>
      ...
      <Link
        ...
        style={{ pointerEvents: isBookingValid ? "auto" : "none", opacity: isBookingValid ? 1 : 0.5 }}
      >
        Mua vé nhanh
      </Link>
    </div>
