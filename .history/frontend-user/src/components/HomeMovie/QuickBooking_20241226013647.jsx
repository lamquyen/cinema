...
const QuickBooking = () => {
  ...
  const fetchShowtimesByMovieId = async (movieId) => {
    try {
      // Lấy thông tin showtimes cho movieId từ API
      ...
      const daysList = showtimesData.reduce((acc, showtime) => {
      ...
      const timesList = showtimesData.reduce((acc, showtime) => {
        if (showtime.days && Array.isArray(showtime.days)) {
          showtime.days.forEach((day) => {
            const formattedDate = new Date(day).toLocaleDateString("vi-VN");
            ...
            // Kiểm tra nếu showtime.times là một mảng
            if (Array.isArray(showtime.times)) {
              // Thêm các times vào ngày tương ứng
              showtime.times.forEach((time) => {
                acc[formattedDate].push(time);
              });
            } else {
              console.error(`Expected 'times' to be an array, but got:`, showtime.times);
            }
          });
        }
        return acc;
...
const handleDateChange = (date) => {
  ...
  // Lấy các times cho ngày đã chọn từ availableTimes
  const filteredTimes = availableTimes[date] || [];
  setAvailableTimes(filteredTimes); // Cập nhật danh sách times cho ngày đã chọn
};
