  ...
  return (
    <div className="flex justify-around w-[100%] items-center h-fit border-b-8 border-gray-200 pb-2 pt-2">
      ...
      <div className="flex justify-between gap-4 items-center h-fit text-gray-600">
        <Dropdown options={options} placeholder="Rạp Chiếu" onSelect={console.log} />
        <Dropdown options={events} placeholder="Sự Kiện" onSelect={console.log} />
        <Dropdown options={movies} placeholder="Phim" onSelect={console.log} />
      ...
      <div>
        ...
        {modalType === "login" && (
          <Login isOpen={true} onClose={handleCloseModal} onLoginSuccess={handleLoginSuccess} />
        )}
        {modalType === "register" && (
          <Register isOpen={true} onClose={handleCloseModal} onLoginClick={handleLoginClick} />
        )}
      </div>
    </div>
  );
