...
const Booking = () => {
    ...
    useEffect(() => {
        ...
        console.log('Dữ liệu nhận được từ API:', data); // Kiểm tra dữ liệu API
                if (response.ok) {
    ...
    // Lấy dữ liệu từ showtimeData với kiểm tra an toàn
    const { movie, room, times, seatsStandard = {}, seatsVIP = {}, priceStandard, priceVIP } = showtimeData;  // Giải nén, nếu không có thì gán mặc định là {}

    // Kiểm tra seatsStandard và seatsVIP để tránh lỗi
    const seatsStandardAvailable = seatsStandard?.available || 0;  // Sử dụng 0 nếu không có
    const seatsVIPAvailable = seatsVIP?.available || 0;  // Sử dụng 0 nếu không có

    // Kiểm tra sự tồn tại của movie trước khi truy cập vào title
    const movieTitle = movie ? movie.title : 'Không rõ tên phim'; // Nếu movie không tồn tại, gán tên mặc định
                        ...
                        <div className="bg-white w-full p-4">
                            <div className="border-dashed border-b-[1px] border-black pb-4 " >
                                <div className="flex flex-start w-fit ">
                                    ...
                                    <div >
                                        <p className="font-bold text-lg">{movieTitle}</p> {/* Sử dụng movieTitle */}
                            ...
                            <div className="mt-4">
                                <p className="flex font-bold">Ghế Standard: {seatsStandardAvailable}/{seatsStandard.total}</p>
                                <p className="flex font-bold">Ghế VIP: {seatsVIPAvailable}/{seatsVIP.total}</p>
                            </div>
