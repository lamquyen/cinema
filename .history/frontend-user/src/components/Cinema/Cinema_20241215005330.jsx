...
function Cinema() {
  ...
  useEffect(() => {
      const fetchCinema = async () => {
        try {
          const response = await axios.get(`http://localhost:5000/api/cinemas/${location}`);  // Gọi API lấy thông tin phim
          setCinema(response.data);  // Lưu thông tin phim vào state
        } catch (error) {
      ...
      fetchCinema();
    }, [location]);  // Gọi lại khi id thay đổi
    ...
    <>
      ...
      <div>
        <div style={{ padding: "20px" }}>
          <CarouselBanner items={cinema.img01} />
        </div>
      ...
      scale-100 blur-0 grayscale-0)" src="https://cdn.galaxycine.vn/media/2023/12/29/banggiave-2024-tan-binh---truong-chinh---kdv---trung-chanh_1703839866233.jpg"></img>
            ...
            <div className="bg-white p-4">
              ...
              <div className="">
                  ...
                  <div className="mt-4">
                      <iframe className="w-full h-[250px]" src={cinema.map}></iframe>
                  </div>
                  <div className="mt-4">
                      <p style={{fontFamily:"sans-serif"}}>{cinema.introduce01}</p>
                      <p >&nbsp;</p>
                      <p style={{fontFamily:"sans-serif"}}> {cinema.introduce02}</p>
                  </div>
              </div>
            </div>
          </div>
