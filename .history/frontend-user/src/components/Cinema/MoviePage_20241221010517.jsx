...
const MoviePage = ({ cinemaLocation }) => {
  ...
  const filteredShowtimes = showtimes.filter((showtime) => {
    if (!showtime.date) {
      console.error("Missing date:", showtime);
      return false; // Loại bỏ nếu không có ngày
    }
  
    const showtimeDate = moment(showtime.date); // ISO 8601 xử lý tự động
    if (!showtimeDate.isValid()) {
      console.error("Invalid date:", showtime.date);
      return false; // Loại bỏ nếu không hợp lệ
    }
  
    return showtimeDate.format("YYYY-MM-DD") === selectedDate; // So sánh ngày
  });
