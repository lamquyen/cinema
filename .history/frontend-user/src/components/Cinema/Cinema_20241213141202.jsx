...
>
  <img src={movie.img} alt={movie.title} className="w-full rounded-md" />
  <h3 className="text-lg font-bold mt-2">{movie.title}</h3>
  <p className="text-sm text-gray-600">Rating: {movie.rating}</p>
  <p className="text-sm text-gray-600">Loại: {movie.type}</p>
</div>
          ))
        ) : (
      ...
      {selectedMovie && (
  <div className="absolute w-full left-0 z-10bg-grey-90 border rounded  px-4 py-6">
